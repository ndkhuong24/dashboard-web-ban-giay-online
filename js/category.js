...
table.addEventListener("click", function (event) {
  if (event.target.id === "capNhat") {
    ...
    if (clickedRow) {
      ...
      const Categoryid = clickedRow.querySelector("td:first-child").textContent;
// Gán giá trị id vào thẻ ẩn trong modal
document.getElementById("modalCategoryId").value = Categoryid;
fetch(`http://localhost:8080/api/Category/id/${Categoryid}`)
.then((response)=>response.json())
.then((CategoryData)=>{
  const UpdateDiv = document.getElementById("updateCategory")
  ...
  let isActiveChecked = "";
    ...
    <form>
             <div class="form-group">
                 ...
                 <input type="text" class="form-control" id="newName" value="${CategoryData.name}">
             </div>
             <fieldset>
    ...
    `;
    UpdateDiv.innerHTML=CategoryHTML;
});
...
// Function để lấy dữ liệu Category bằng ID
function fetchCategoryById(Categoryid, callback) {
  // Đường dẫn API với ID
  const apiUrl = `http://localhost:8080/api/Category/id/${Categoryid}`;
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      // Gọi hàm callback với dữ liệu Category
      callback(data);
    })
    .catch((error) => {
      showNotification("Đã xảy ra lỗi");
...
document.getElementById("confirmUpdate").addEventListener("click", function () {
  ...
  const CategoryIdFromModal = document.getElementById("modalCategoryId").value;

  // Gọi hàm để lấy dữ liệu Style bằng ID
  fetchCategoryById(CategoryIdFromModal, function (categoryData) {
...
function searchByName(searchPattern) {
  // Lấy dữ liệu từ API và render trang đầu tiên
  fetch(`http://localhost:8080/api/Category/${searchPattern}`)
    .then((response) => response.json())
    .then((searhData) => {
      currentPage = 1;
      data = searhData;
