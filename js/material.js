...
function fetchMatrerialById(Materialid, callback) {
  // Đường dẫn API với ID
  const apiUrl = `http://localhost:8080/api/Material/id/${Materialid}`;
...
document.getElementById("confirmUpdate").addEventListener("click", function () {
  ...
  const MaterialIdFromModal = document.getElementById("modalMaterialId").value;
...
function searchByName(searchPattern) {
  // Lấy dữ liệu từ API và render trang đầu tiên
  fetch(`http://localhost:8080/api/Material/${searchPattern}`)
    .then((response) => response.json())
    .then((searhData) => {
      currentPage = 1;
      data = searhData;
