...
table.addEventListener("click", function (event) {
  if (event.target.id === "capNhat") {
    ...
    if (clickedRow) {
      ...
      const Colorid = clickedRow.querySelector("td:first-child").textContent;
   // Gán giá trị id vào thẻ ẩn trong modal
   document.getElementById("modalColorId").value = Colorid;
   fetch(`http://localhost:8080/api/Color/id/${Colorid}`)
   .then((response)=>response.json())
   .then((ColorData)=>{
     const UpdateDiv = document.getElementById("updateColor")
     ...
     let isActiveChecked = "";
       ...
       <form>
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="newName" value="${ColorData.name}">
                </div>
                <fieldset>
...
function fetchColorById(ColorId, callback) {
  // Đường dẫn API với ID
  const apiUrl = `http://localhost:8080/api/Color/id/${ColorId}`;
...
document.getElementById("confirmUpdate").addEventListener("click", function () {
  ...
  const ColorIdFromModal = document.getElementById("modalColorId").value;

  // Gọi hàm để lấy dữ liệu Style bằng ID
  fetchColorById(ColorIdFromModal, function (colorData) {
    const newId = colorData.id;
...
function searchByName(searchPattern) {
  // Lấy dữ liệu từ API và render trang đầu tiên
  fetch(`http://localhost:8080/api/Color/${searchPattern}`)
    .then((response) => response.json())
    .then((searhData) => {
      currentPage = 1;
      data = searhData;
